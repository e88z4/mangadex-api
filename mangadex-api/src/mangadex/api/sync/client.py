...
class MangaDexClient:
    ...
    def get_chapter(self, chapter_id):
        self.ensure_authenticated()
        headers = self._get_auth_headers()
        response = self.session.get(f'{self.BASE_URL}/chapter/{chapter_id}', headers=headers)
        return response.json()

    def create_custom_list(self, list_data):
        self.ensure_authenticated()
        headers = self._get_auth_headers()
        response = self.session.post(f'{self.BASE_URL}/list', json=list_data, headers=headers)
        return response.json()

    def update_custom_list(self, list_id, list_data):
        self.ensure_authenticated()
        headers = self._get_auth_headers()
        response = self.session.put(f'{self.BASE_URL}/list/{list_id}', json=list_data, headers=headers)
        return response.json()

    def delete_custom_list(self, list_id):
        self.ensure_authenticated()
        headers = self._get_auth_headers()
        response = self.session.delete(f'{self.BASE_URL}/list/{list_id}', headers=headers)
        return response.json()